...
class DownloaderBloc extends Bloc<DownloaderEvent, DownloaderState> {
  ...
  FutureOr<void> _downloadInfo(
      GetDownloadItemInfo event, Emitter<DownloaderState> emit,) async {
  ...
  FutureOr<void> _downloadItem(
      DownloadItem event, Emitter<DownloaderState> emit,) async {
    ...
    data.fold((l) => emit(DownloadingError(l.errorMessage)), DownloadingItems.new);
  }
}
